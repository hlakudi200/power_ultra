...
CREATE TABLE public.notifications (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  type text NOT NULL CHECK (type = ANY (ARRAY['class_cancelled'::text, 'class_rescheduled'::text, 'booking_confirmed'::text, 'membership_expiring'::text, 'general'::text, 'waitlist_spot_available'::text, 'waitlist_expired'::text, 'waitlist_joined'::text])),
  title text NOT NULL,
  message text NOT NULL,
  related_id uuid,
...
CREATE TABLE public.trainer_assignments (
  ...
  CONSTRAINT trainer_assignments_member_id_fkey FOREIGN KEY (member_id) REFERENCES public.profiles(id),
  CONSTRAINT trainer_assignments_trainer_id_fkey FOREIGN KEY (trainer_id) REFERENCES public.instructors(id),
  CONSTRAINT trainer_assignments_assigned_by_fkey FOREIGN KEY (assigned_by) REFERENCES public.profiles(id)
);
CREATE TABLE public.trainer_client_notes (
  ...
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT trainer_client_notes_pkey PRIMARY KEY (id),
  CONSTRAINT trainer_client_notes_assignment_id_fkey FOREIGN KEY (assignment_id) REFERENCES public.trainer_assignments(id),
  CONSTRAINT trainer_client_notes_trainer_id_fkey FOREIGN KEY (trainer_id) REFERENCES public.instructors(id)
);
CREATE TABLE public.waitlist (
  ...
  status text NOT NULL DEFAULT 'waiting'::text CHECK (status = ANY (ARRAY['waiting'::text, 'notified'::text, 'converted'::text, 'expired'::text, 'cancelled'::text])),
  ...
  CONSTRAINT waitlist_schedule_id_fkey FOREIGN KEY (schedule_id) REFERENCES public.schedule(id),
  CONSTRAINT waitlist_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.profiles(id)
);
CREATE TABLE public.waitlist_cleanup_log (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  cleanup_date date NOT NULL DEFAULT CURRENT_DATE,
  entries_deleted integer NOT NULL,
  cleanup_type text NOT NULL CHECK (cleanup_type = ANY (ARRAY['automatic'::text, 'manual'::text])),
  performed_by uuid,
  details jsonb,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT waitlist_cleanup_log_pkey PRIMARY KEY (id),
  CONSTRAINT waitlist_cleanup_log_performed_by_fkey FOREIGN KEY (performed_by) REFERENCES public.profiles(id)
);
CREATE TABLE public.workout_exercises (
  ...
  exercise_type text CHECK (exercise_type = ANY (ARRAY['strength'::text, 'cardio'::text, 'flexibility'::text, 'sports'::text, 'other'::text])),
  sets integer,
  reps text,
  weight text,
  ...
  CONSTRAINT workout_exercises_plan_id_fkey FOREIGN KEY (plan_id) REFERENCES public.workout_plans(id)
);
CREATE TABLE public.workout_plans (
  ...
  CONSTRAINT workout_plans_assignment_id_fkey FOREIGN KEY (assignment_id) REFERENCES public.trainer_assignments(id),
  CONSTRAINT workout_plans_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.instructors(id)
);
CREATE TABLE public.workout_progress (
  ...
  CONSTRAINT workout_progress_exercise_id_fkey FOREIGN KEY (exercise_id) REFERENCES public.workout_exercises(id),
  CONSTRAINT workout_progress_member_id_fkey FOREIGN KEY (member_id) REFERENCES public.profiles(id),
  CONSTRAINT workout_progress_plan_id_fkey FOREIGN KEY (plan_id) REFERENCES public.workout_plans(id)
);